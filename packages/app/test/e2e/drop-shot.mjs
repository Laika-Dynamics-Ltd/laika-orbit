...
const gift = join(dir, 'brand-guide.pdf')
writeFileSync(gift, Buffer.alloc(2_400_000, 7))
...
// the other Orbit offers this machine a file; nothing is written until the panel says yes
const incoming = them.sendFile({ id: 'mine', name: 'this-mac', host: '127.0.0.1', port: dropPort }, gift, { poll: 300 }).catch((e) => console.log('  (offer ended:', e.message, ')'))

await page.keyboard.press('Alt+Meta+KeyD')
await page.waitForSelector('#pnl-drop .dz-zone', { timeout: 15_000 })
await page.waitForTimeout(1500)

// drag a file onto the panel, the way the pointer would
await page.evaluate(() => {
  const dt = new DataTransfer()
  dt.items.add(new File([new Uint8Array(840_000)], 'kickoff-deck.key', { type: 'application/octet-stream' }))
